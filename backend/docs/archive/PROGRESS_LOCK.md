...
- 2026-02-11: Admin UI V3 Phase C Diena 2 — Skambučiai ir Kalendorius: call-requests stats (new_count), filter chips „Nauji“ (default su badge), kortelės (Global Attention), SSE call_request_created, AI summary. Appointments stats (pending_schedule_count), mini-triage (HELD), POST confirm-hold, 2 veiksmai (Patvirtinti, Pasiūlyti kitą laiką).

- 2026-02-11: Admin UI V3 Phase C Diena 1 — Projektai: GET /admin/projects/view, GET /admin/projects/mini-triage, build_projects_view, build_projects_mini_triage, filter chips (Laukiantys veiksmo default), mini triage kortelės, row-urgency-*, PRIMARY mygtukas, AI summary pill, quickAction (assign_expert, certify_project), 5 pytest (view model, attention filter, mini-triage, cursor/as_of, cursor mismatch 400). Docs: ADMIN_UI_V3.md, API_ENDPOINTS_CATALOG.md, PROGRESS_LOCK.md.

- 2026-02-11: Admin UI V3 Phase C Diena 3 — Auditas ir Maržos: audit.html — filter chips (7 d.+El.paštas default), default 7d+SYSTEM_EMAIL, AI summary pill (įrašų/klaidų), applyChipFilter, initFilterChips. margins.html — rules_version, recent_changes_count API, Global Attention (pastarų 7d pakeitimai), AI summary pill, preview calc (nauja taisyklė). MarginListResponse: rules_version, recent_changes_count.

- 2026-02-11: Admin UI V3 Phase C Diena 4 — Finansai ir AI: GET /admin/finance/view, GET /admin/finance/mini-triage, build_finance_view, build_finance_mini_triage (laukiantys mokėjimai — DRAFT be depozito, CERTIFIED be final). finance.html — sidebar token, mini triage, AI summary pill, SSE metrics kortelės viršuje, quickAction (record_deposit, record_final). GET /admin/ai/view, build_ai_view (low_confidence_count, attention_items). ai-monitor.html — sidebar token, Global Attention (žemi confidence), AI summary „Patikrinti N klaidų“. renderMiniTriage reusable JS. 3 pytest (test_admin_finance_ai_d4).

- 2026-02-11: Admin UI V3 Phase C Diena 5–6 — Token unifikacija, Global search: visi admin puslapiai (audit, calls, calendar, margins, projects) — token perkeltas į sidebar (sidebar-token). GET /admin/search?q= (admin_search.py) — ieško projektų, skambučių pagal ID/status, max 50, LOCK 1.4 (loginti tik q ilgį). Sidebar viršuje search input (Ctrl+K). Docs: API_ENDPOINTS_CATALOG, ADMIN_UI_V3.
